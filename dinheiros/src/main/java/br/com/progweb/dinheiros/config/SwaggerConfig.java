...
public class SwaggerConfig {
	...
	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title(TITLE).description(DESC)
				.contact(new Contact("m, r", HOMEPAGE, "m.gregorio@aluno.ufabc.edu.br")).version(SWAGGER_API_VERSION)
				.build();
	...
	public Docket productApi() {
		return new Docket(DocumentationType.SWAGGER_2).ignoredParameterTypes().apiInfo(apiInfo()).pathMapping("/")
				.select().paths(PathSelectors.any())// regex("/.*/v.*/.*"))
